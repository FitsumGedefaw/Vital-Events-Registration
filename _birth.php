...
// qouery the databse
$statement = $pdo->prepare('SELECT * FROM birth_table WHERE  p_ssn = :ssn');
$statement->bindValue(':ssn', $ssn);
$statement->execute();
$birth_info = $statement->fetchAll(PDO::FETCH_ASSOC);
...
<body>
    ...
    <div class="person-container">
        ...
        <!--! info begins here -->
        <?php if (empty($birth_info)): ?>

        <div class="no-personal-info">
            Birth information is not provided yet.
        </div>
        <?php endif; ?>

        <?php foreach ($birth_info as $i => $element) {?>
        <div class="person-info">
            ...
            <div class="p-ssn">
                <p>SSN : <?php echo $element['b_ssn'] ?> </p>
            </div>
            ...
            <div class="p-actions">
                ...
                <!-- for update -->
                <form action="B_edit-birth.php">
                    <input type="hidden" name="edit" value="<?php echo $element['b_ssn']?>" />
                    <button class="edit" type="submit">Edit</button>
                </form>

                <!-- for delete -->
                <form method="post" action="C_delete-birth.php">
                    <input type="hidden" name="delete" value="<?php echo $element['b_ssn']?>" />
